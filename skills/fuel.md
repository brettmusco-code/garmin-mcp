...
Related tools: **`get_adaptive_tdee`** (measured maintenance from intake vs weight change), **`get_race_fueling`** (race-day calculator), **`set_race` / `get_races` / `delete_race`** (the race calendar — the plan periodizes itself around it), **`push_nutrition_targets_to_garmin`** (write goals into the Garmin app). The plan output also carries a **`projection`** (forward weight curve + finish date), a per-day **`meals[]`** split, **`energy_base`** (bmr_x1.3 or adaptive_tdee), and heat-aware / swim-aware fuel cards. Swims never get pre/during fuel (post only); outdoor sessions get a fluid/sodium bump on hot days (needs `home_lat`/`home_lon` on the goal, or a `heat_c` override).
...
2. **`generate_fueling_plan(days = {days or 7}, rebalance = true)`** — the engine. `rebalance=true` self-corrects from the week so far: the net intake error on logged days (vs their expenditure-adjusted targets) is spread across this window, so over-eating Tuesday tightens Wed–Sun and a banked deficit loosens them. Returns everything needed:
   ...
   - `bmr` (`value`, `source` — `katch_mcardle` when a body-fat reading is present, else `mifflin_st_jeor`, else `weight_x22_fallback`), `energy_base` (`value`, `source`, `model`), `body_fat_pct`, `daily_kcal_adjustment`, `protein_g_per_kg` (base)
   - `weight_trend` — the weigh-in-trend calibration: measured vs expected kg/week, `confidence`, an `applied_factor` on the base when it auto-corrects, and a `note` (surface it — when confidence is `none` it's telling you to sync weight more often)
   - **Energy model** (explain if asked): expenditure = **RMR × 1.15 (NEAT)** + **net exercise** (each session's **Active Calories** — gross burn minus the resting calories you'd burn anyway over those hours, matching Garmin's own Active/Resting split — no double-count) + **TEF** (thermic effect of food, ~10–15%, protein-weighted). Once food logging is consistent, `energy_base.source` switches to `adaptive_tdee` (measured maintenance) and TEF isn't added on top. `est_burn_kcal` and `net_exercise_kcal` are now the same Active-Calories figure — both "burned" and energy-availability only count workout exercise energy, never a resting baseline for the workout's duration.
   - **Today uses real burn**: for the current day, a session that's already been completed has its estimate swapped for the **actual** Active Calories logged from Garmin (`sessions[].burn_source: "actual_today"`, `done: true`) and unplanned workouts are folded in — so today's target and EA track reality as the day unfolds. Sessions not yet done stay estimated (`done: false`). Combined with the hourly cron refresh, today's numbers move with your real training and eating.
   - **Burned vs projected**: every day row splits Active-Calorie burn into `burned_kcal` (sum of completed/`done` sessions) and `projected_burn_kcal` (everything still to come). On past/future days nothing is done, so `burned_kcal` is 0 and the whole burn is projected; mid-day today it reads e.g. *burned 950 · +620 projected = 1,570*. Surface the split rather than a single burn figure when a day is in progress.
   - `days[]` — per day: `sessions[]` (each with `kcal_per_hour` + `burn_source`), `primary_intensity`, `est_burn_kcal` (workout Active Calories), `net_exercise_kcal`, `tef_kcal`, `target_kcal`, **`target_deficit_kcal`** (expenditure − target; >0 = deficit), `protein_g` / `carbs_g` / `fat_g`, **`protein_g_per_kg`** (that day's periodized value), `carb_g_per_kg`, `energy_availability_kcal_per_kg_ffm`, `needs_fuel`, and `fuel[]` cards
   ...
   - **Deficit periodization** (default for lose goals): the weekly deficit is banked on rest/easy days while hard days stay near maintenance — each day reports its own `kcal_adjustment`. A day's capacity to absorb a cut is scored by **training load as TSS** (`hours × IF² × 100`, one threshold hour = 100), not just its hardest session's label: intensity counts non-linearly, so a genuinely big aerobic day (e.g. 2h+ of easy work, ~90 TSS) is protected like a hard day rather than mislabeled "easy." Any session categorically hard (tempo/threshold/VO2/long) or any day whose cumulative TSS clears the protection threshold never takes a deeper cut than the flat per-day amount. `periodize_deficit=false` restores a flat daily deficit. When floors bind and part of the weekly deficit can't be absorbed, a shortfall note fires — surface it.
   - **Phantom days**: a run of **≥2 consecutive days with nothing on the calendar** is treated as not-yet-scheduled training rather than genuine rest — each is planned as a typical training day (mean net burn + hours over the last 30 days, `burn_source: assumed_30d_avg`, flagged `assumed` on the session) and protected from a deep-deficit cut, so a later-synced workout doesn't land on a day that was starved. Isolated single open days stay true rest. A note lists the assumed dates — surface it.
   ...
   - **How macros work** (explain if asked): protein = **goal weight** × a per-kg factor (base **2.2 g/kg** on a cut — anchored to target weight so it holds steady as you lean out instead of drifting down with the scale), nudged up +0.2 g/kg on threshold/VO2/long days, +0.1 on tempo, +0.1 in a steep (≥500) deficit; carbs = current bodyweight × a factor periodized by session type (3→8 g/kg), and carbs are the **flex** macro that absorbs the extra on a big training day; fat fills the gap but is **capped at ~30% of calories** (`carbs_trimmed` flips only on the opposite case — a deficit so steep the target can't hold the training carbs). Session burn is calibrated from the athlete's own history — same sport **and** similar duration first (`burn_source: history_similar`), then sport median, then a generic table.
   - Fuel cards are emitted only for sessions **≥ `fuel_min_minutes` (default 90)**.
   - **Race periodization** (automatic once races are stored — see step 4b): each day in a race window carries a `race` object (`name`, `date`, `phase`, `days_until`, `label`) and the plan has already applied it. Phases: `taper` (deficit at 50%, then 25% inside 3 days, **and energy availability floored at 40 kcal/kg FFM** — on an aggressive cut the floor is what actually binds, since tapering near EA 20 measurably *depletes* muscle glycogen while EA 45 leaves it alone; capped at maintenance, so the taper never forces a surplus), `carb_load` (deficit **off**, carbs ramp 8 → 10 g/kg over the last 1–3 days), `race_day` (deficit off, carbs set by the race fuel card, the race injected as a session with its own estimated burn), `recovery` (day after: deficit off, 6 g/kg carbs, +0.3 g/kg protein), `post_race` (deficit at 50%). Breakfast-skip is suppressed on loading and race days. Every phase adds a `notes[]` entry — surface them.
   - **Everything scales off estimated duration, not distance** — which is the axis that matters physiologically, and is why a fast 10K and a slow half can land in different bands. Loading days: **none under ~90 min** (resting glycogen already covers the event — loading is water and digestive load for no gain), 1 to 3 h, 2 beyond, all at a flat **10 g/kg**. Deliberately short: maximal glycogen is reached within 24 h at 10 g/kg and then holds, and 8 g/kg is statistically indistinguishable from 6 — so a 3-day 8→10→10 ramp spends two extra surplus days for no extra glycogen. An Ironman and a marathon load identically, and that's correct. What keeps scaling is recovery (1 / 2 / 3 days, then **5** past 5 h and **7** past 8 h) and race-day intake, which is set by the fuel card and so tracks the event directly: ~0 g/hr for a 5K, 45 g/hr for a sprint tri, 90 g/hr × 3.5 h for a marathon, 90 g/hr × 10.6 h for an Ironman. **Taper length is duration-driven too** (1 day under an hour, 2 to 90 min, 3 to 3 h, 7 beyond), with priority as a cap only: A ≤ 7 days, B ≤ 4, C none. So an A-priority sprint tapers for 2 days, not a marathon's 7 — giving a short race a week of halved deficit is how a cut quietly loses a kilo of progress to a 70-minute event.
   - **Loading days run ABOVE maintenance on purpose.** 10 g/kg for a 74 kg athlete is 740 g of carbohydrate — ~2,960 kcal before any protein or fat — so on a tapered week the day is a surplus by construction. Those days are floored at what their own prescribed macros cost, rather than trimming the carbs back to fit maintenance (which would make the ramp cosmetic). A note fires; surface it, and steer toward low-fat, low-fibre, energy-dense carbs — trying to hit these numbers on vegetables and whole grains is how loading weeks go wrong.
   - Pass `carb_load=true` to force the whole window into loading mode by hand — a blunt override that suspends the deficit and raises carbs to ~9 g/kg on **every** day of the window. Prefer storing the race (step 4b): it periodizes precisely and needs no re-passing. Config overrides (all optional): `max_deficit_kcal` (0 removes the cap), `ea_floor` (0 disables the warning), `ea_min` (enforced EA floor — the best guardrail when cutting hard, since it protects training days automatically), `min_kcal` (absolute floor), `bmr_floor_mult` (0 drops the BMR floor), `periodize_deficit`, `front_load` (0–0.9: steeper deficit early, tapering as weight nears target — recomputed from each weigh-in so it self-tapers), `fuel_min_minutes`, `skip_breakfast_weekdays` (time-restricted eating: drop the weekday breakfast, keep weekends). These also persist on the goal via `set_fueling_goal`. When the goal pace exceeds what the floors allow, the shortfall note projects the realistic landing date — always surface it.
...
4b. **Races:** if I mention an upcoming event, **store it** — `set_race(date, name, distance_label or distance+distance_unit, priority, [target_time_hours], [hot])`. From then on every `generate_fueling_plan` builds the taper, carb load, race-day fuelling and post-race recovery around it automatically; nothing needs re-passing. **Priority defaults to B** (tune-up) rather than A, because A costs the most progress on a cut and should be deliberate — so ask which it is if I don't say (A = goal race, B = tune-up, C = trained through) and confirm the estimated duration it comes back with — that number decides how many loading days the race earns, so correct it with `target_time_hours` if it looks off (`duration_source: default_pace` means it's a guess from a generic table). Use `get_races` to list what's stored (each entry has an `id`) and `delete_race(race_id)` to remove one.
   - For the race-day detail itself — pre-race meal, hour-by-hour carbs/fluid/sodium/caffeine, gel count, the loading protocol — call `get_race_fueling(race_id=...)`, which reads sport, duration and heat straight off the stored race. For a hypothetical event that isn't on the calendar, pass `sport` + `duration_hours` instead.
...
{if races[]: Next race: {name} — {date} ({days_until}d), {distance_label}, est {duration_hours}h · {carb_load_days} load days}
...
{mark any day with a `race` phase inline — e.g. "🏁 Race day" / "Carb load · 2d out" / "Taper" after the session name}
...
- Never invent a race either — only `set_race` what I actually told you, and say the estimated duration back to me rather than presenting it as fact. When a day sits in a race phase, say *why* its deficit is off ("carb load, 2 days out") rather than leaving it looking like the plan lost the cut.

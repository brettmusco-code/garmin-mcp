...
2. **`generate_fueling_plan(days = {days or 7}, rebalance = true)`** — the engine. `rebalance=true` self-corrects from the week so far: the net intake error on logged days (vs their expenditure-adjusted targets) is spread across this window, so over-eating Tuesday tightens Wed–Sun and a banked deficit loosens them. Returns everything needed:
   ...
   - **Energy model** (explain if asked): expenditure = **RMR × 1.15 (NEAT)** + **net exercise** (each session's gross burn minus the resting calories you'd burn anyway over those hours — no double-count) + **TEF** (thermic effect of food, ~10–15%, protein-weighted). Once food logging is consistent, `energy_base.source` switches to `adaptive_tdee` (measured maintenance) and TEF isn't added on top. Each day carries `net_exercise_kcal` and `tef_kcal`; `est_burn_kcal` stays **gross** so it matches Garmin/your watch.
   - **Today uses real burn**: for the current day, a session that's already been completed has its estimate swapped for the **actual** logged calories from Garmin (`sessions[].burn_source: "actual_today"`, `done: true`) and unplanned workouts are folded in — so today's target and EA track reality as the day unfolds. Sessions not yet done stay estimated (`done: false`). Combined with the hourly cron refresh, today's numbers move with your real training and eating.
   - **Burned vs projected**: every day row splits gross burn into `burned_kcal` (sum of completed/`done` sessions) and `projected_burn_kcal` (everything still to come). On past/future days nothing is done, so `burned_kcal` is 0 and the whole burn is projected; mid-day today it reads e.g. *burned 950 · +620 projected = 1,570*. Surface the split rather than a single burn figure when a day is in progress.
   - `days[]` — per day: `sessions[]` (each with `kcal_per_hour` + `burn_source`), `primary_intensity`, `est_burn_kcal` (gross), `net_exercise_kcal`, `tef_kcal`, `target_kcal`, **`target_deficit_kcal`** (expenditure − target; >0 = deficit), `protein_g` / `carbs_g` / `fat_g`, **`protein_g_per_kg`** (that day's periodized value), `carb_g_per_kg`, `energy_availability_kcal_per_kg_ffm`, `needs_fuel`, and `fuel[]` cards

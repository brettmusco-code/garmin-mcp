    ...
    },
    {
        ...
        "inputSchema": {
            ...
            "properties": {
                ...
                "aggressive": {"type": "boolean", "description": "hold the MAX sustainable deficit (bounded by the EA/BMR floors) instead of pacing to target_date, so losing weight moves the finish date EARLIER. The per-day energy-availability floor still applies. Default off."},
                "rebalance_deficit_only": {"type": "boolean", "description": "make the rolling rebalance one-directional: only TIGHTEN targets to pay back an overage, never RAISE them to give calories back after undereating. Keeps good days from eroding the deficit. Default off (two-way)."},
    ...
    },
    {
        ...
        "inputSchema": {
            ...
            "properties": {
                ...
                "priority": {"type": "string", "description": "A (goal race) | B (tune-up) | C (load days only). Default B \u2014 A costs the most cut progress, so it should be chosen deliberately. Duration sets the taper window and priority caps it (A<=7d, B<=4d), so an A-race sprint tapers 2 days, not 7."},
...
def _call_tool(name: str, args: dict) -> Any:
    ...
    if name == "set_fueling_goal":
        ...
        return garmin.set_fueling_goal(
            ...
            aggressive=args.get("aggressive"),
            rebalance_deficit_only=args.get("rebalance_deficit_only"),
